...
func TestIsBalanced_simple_not_balanced(t *testing.T) {
	...
	if assert.ShouldBeFalse(t, balanced) {
		assert.Equal(t, err.Error(), "unbalanced value of -0.100000 for GBP")
	}
}
...
func TestEnsureBalanced_basic(t *testing.T) {
	...
	assert.Equal(t, transaction.Lines[1].Value.Value.Text('f', 3), "0.200")
}
...
func TestEnsureBalanced_multiple_no_change(t *testing.T) {
	transaction := &Transaction{
		Lines: []TransactionLine{
			{Account: "From", Value: &Amount{Value: big.NewFloat(-0.2), Commodity: "GBP"}},
			{Account: "From", Value: &Amount{Value: big.NewFloat(0.2), Commodity: "ZAR"}},
			{Account: "To", Value: &Amount{Value: big.NewFloat(0.2), Commodity: "GBP"}},
			{Account: "To", Value: &Amount{Value: big.NewFloat(-0.2), Commodity: "ZAR"}},
		},
	}
	changed, err := EnsureBalanced(transaction)
...
func TestEnsureBalanced_addition_of_values(t *testing.T) {
	...
	assert.Equal(t, changed, true)
	assert.Equal(t, err, nil)
	assert.Equal(t, transaction.Lines[2].Value.Value.Text('f', 3), "0.400")
}
...
func TestEnsureBalanced_multiple_commodities(t *testing.T) {
	...
	assert.Equal(t, changed, true)
	assert.Equal(t, err, nil)
	assert.Equal(t, transaction.Lines[2].Value.Value.Text('f', 3), "0.200")
	assert.Equal(t, transaction.Lines[3].Value.Value.Text('f', 3), "-0.200")
}
...
func TestEnsureBalanced_bad_balance(t *testing.T) {
	transaction := &Transaction{
		Lines: []TransactionLine{
			{Account: "From", Value: &Amount{Value: big.NewFloat(-0.2), Commodity: "GBP"}},
			{Account: "To", Value: &Amount{Value: big.NewFloat(0.3), Commodity: "GBP"}},
		},
	}
	changed, err := EnsureBalanced(transaction)
	assert.Equal(t, changed, false)
	assert.Equal(t, err, fmt.Errorf("unbalanced value of 0.100000 for GBP"))
}
